...
class DahuaClient:
	...
	def __init__(
		...
		self.rpc2_client = DahuaRpc2Client(self._username, self._password, self._address, self._port, self._rtsp_port, self._session)
	...
	async def toggle_light(self):
		response: CoaxialControlIOStatus = await self.rpc2_client.set_coaxial_control_io_status(0, 1, 1, 2)
		if response.white_light == True:
			return True
		else:
			_LOGGER.warning("Error turning light on")

	async def toggle_siren(self):
		response: CoaxialControlIOStatus = await self.rpc2_client.set_coaxial_control_io_status(0, 2, 1, 2)
		if response.speaker == True:
			return True
		else:
			_LOGGER.warning("Error turning siren on")
	...
	async def post(self, url: str, payload: dict) -> dict:
		...
		try:
			...
			_LOGGER.warning("TimeoutError posting information to %s", url)
			raise exception
		except (KeyError, TypeError) as exception:
			_LOGGER.warning("TypeError posting information to %s", url)
			raise exception
		except (aiohttp.ClientError, socket.gaierror) as exception:
			_LOGGER.debug("ClientError posting information to %s", url)
			raise exception
		except Exception as exception:  # pylint: disable=broad-except
			_LOGGER.warning("Exception posting information to %s", url)
			raise exception
