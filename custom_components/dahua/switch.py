...
async def async_setup_entry(hass: HomeAssistant, entry, async_add_devices):
    ...
    devices = [
        ...
        DahuaHumanSmartMotionDetectionBinarySwitch(coordinator, entry),
        DahuaVehicleSmartMotionDetectionBinarySwitch(coordinator, entry)
    ]
...
class DahuaHumanSmartMotionDetectionBinarySwitch(DahuaBaseEntity, SwitchEntity):
    """Enables or disables the Smart Motion Detection option in the camera"""

    async def async_turn_on(self, **kwargs):  # pylint: disable=unused-argument
        """Turn on SmartMotionDetect"""
        await self._coordinator.client.async_set_ivs_rule(0, 1, True)
        await self._coordinator.async_refresh()

    async def async_turn_off(self, **kwargs):  # pylint: disable=unused-argument
        """Turn off SmartMotionDetect"""
        await self._coordinator.client.async_set_ivs_rule(0, 1, False)
        await self._coordinator.async_refresh()

    @property
    def name(self):
        """Return the name of the switch."""
        return self._coordinator.get_device_name() + " " + "Human Smart Motion Detection"
    ...
    def unique_id(self):
        ...
        https://developers.home-assistant.io/docs/entity_registry_index/#unique-id-requirements
        """
        return self._coordinator.get_serial_number() + "_human_smart_motion_detection"
    ...
    def is_on(self):
        """ Return true if the switch is on. """
        return self._coordinator.is_human_smart_motion_detection_enabled()

class DahuaVehicleSmartMotionDetectionBinarySwitch(DahuaBaseEntity, SwitchEntity):
    """Enables or disables the Smart Motion Detection option in the camera"""

    async def async_turn_on(self, **kwargs):  # pylint: disable=unused-argument
        """Turn on SmartMotionDetect"""
        await self._coordinator.client.async_set_ivs_rule(0, 2, True)
        await self._coordinator.async_refresh()

    async def async_turn_off(self, **kwargs):  # pylint: disable=unused-argument
        """Turn off SmartMotionDetect"""
        await self._coordinator.client.async_set_ivs_rule(0, 2, False)
        await self._coordinator.async_refresh()

    @property
    def name(self):
        """Return the name of the switch."""
        return self._coordinator.get_device_name() + " " + "Vehicle Smart Motion Detection"
    ...
    def unique_id(self):
        ...
        https://developers.home-assistant.io/docs/entity_registry_index/#unique-id-requirements
        """
        return self._coordinator.get_serial_number() + "_vehicle_smart_motion_detection"
    ...
    def is_on(self):
        """ Return true if the switch is on. """
        return self._coordinator.is_vehicle_smart_motion_detection_enabled()
